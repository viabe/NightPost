...
fix(ui): 인벤토리 버튼 오류 수정
refactor(enemy): FSM 구조 개선
asset(environment): 숲 오브젝트 추가
scene(stage1): Stage1 레벨 수정
```
...
- Commit Type은 소문자를 사용합니다.
- 제목(subject)은 **50자 이내**로 작성합니다.
- 제목 끝에는 마침표(`.`)를 붙이지 않습니다.
- 제목은 **무엇을 변경했는지** 간결하게 작성합니다.
- 여러 작업을 하나의 커밋에 포함하지 않습니다.
- 필요한 경우 Body에 변경 이유를 작성합니다.
...
fix(enemy): 보스 공격 판정 오류 수정

refactor(inventory): 아이템 관리 클래스 분리

asset(character): 기사 캐릭터 모델 추가

anim(player): 대시 애니메이션 추가

scene(tutorial): 튜토리얼 맵 수정

audio(boss): 보스 BGM 추가

shader(water): 물 셰이더 개선

ui(settings): 옵션 창 UI 수정

network(room): 룸 생성 기능 구현

docs(readme): Git Commit Convention 추가
```
